from PIL import Image
from colors import colorsDict
from traitEncodings import TRAIT_ENCODINGS

def drawTophat(im, primaryColor, primaryColorShade, strapColor, strapColorShade, buckleColor):
    imNew = Image.new('RGBA', (24, 24), (0, 0, 0, 0))

    imNew.putpixel((7, 1), colorsDict["black"])
    imNew.putpixel((8, 1), colorsDict["black"])
    imNew.putpixel((9, 1), colorsDict["black"])
    imNew.putpixel((10, 1), colorsDict["black"])
    imNew.putpixel((11, 1), colorsDict["black"])
    imNew.putpixel((12, 1), colorsDict["black"])
    imNew.putpixel((13, 1), colorsDict["black"])
    imNew.putpixel((14, 1), colorsDict["black"])
    imNew.putpixel((15, 1), colorsDict["black"])
    imNew.putpixel((16, 1), colorsDict["black"])

    imNew.putpixel((6, 2), colorsDict["black"])
    imNew.putpixel((7, 2), colorsDict[primaryColorShade])
    imNew.putpixel((8, 2), colorsDict[primaryColor])
    imNew.putpixel((9, 2), colorsDict[primaryColor])
    imNew.putpixel((10, 2), colorsDict[primaryColor])
    imNew.putpixel((11, 2), colorsDict[primaryColor])
    imNew.putpixel((12, 2), colorsDict[primaryColor])
    imNew.putpixel((13, 2), colorsDict[primaryColor])
    imNew.putpixel((14, 2), colorsDict[primaryColor])
    imNew.putpixel((15, 2), colorsDict[primaryColor])
    imNew.putpixel((16, 2), colorsDict[primaryColor])
    imNew.putpixel((17, 2), colorsDict["black"])

    imNew.putpixel((6, 3), colorsDict["black"])
    imNew.putpixel((7, 3), colorsDict[primaryColorShade])
    imNew.putpixel((8, 3), colorsDict[primaryColor])
    imNew.putpixel((9, 3), colorsDict[primaryColor])
    imNew.putpixel((10, 3), colorsDict[primaryColor])
    imNew.putpixel((11, 3), colorsDict[primaryColor])
    imNew.putpixel((12, 3), colorsDict[primaryColor])
    imNew.putpixel((13, 3), colorsDict[primaryColor])
    imNew.putpixel((14, 3), colorsDict[primaryColor])
    imNew.putpixel((15, 3), colorsDict[primaryColor])
    imNew.putpixel((16, 3), colorsDict[primaryColor])
    imNew.putpixel((17, 3), colorsDict["black"])

    imNew.putpixel((5, 4), colorsDict["solanaBand"][4])
    imNew.putpixel((6, 4), colorsDict["black"])
    imNew.putpixel((7, 4), colorsDict[primaryColorShade])
    imNew.putpixel((8, 4), colorsDict[primaryColor])
    imNew.putpixel((9, 4), colorsDict[primaryColor])
    imNew.putpixel((10, 4), colorsDict[primaryColor])
    imNew.putpixel((11, 4), colorsDict[primaryColor])
    imNew.putpixel((12, 4), colorsDict[primaryColor])
    imNew.putpixel((13, 4), colorsDict[primaryColor])
    imNew.putpixel((14, 4), colorsDict[primaryColor])
    imNew.putpixel((15, 4), colorsDict[primaryColor])
    imNew.putpixel((16, 4), colorsDict[primaryColor])
    imNew.putpixel((17, 4), colorsDict["black"])
    imNew.putpixel((18, 4), colorsDict["solanaBand"][4])

    imNew.putpixel((4, 5), colorsDict["solanaBand"][5])
    imNew.putpixel((5, 5), colorsDict["solanaBand"][5])
    imNew.putpixel((6, 5), colorsDict["black"])
    imNew.putpixel((7, 5), colorsDict[primaryColorShade])
    imNew.putpixel((8, 5), colorsDict[primaryColor])
    imNew.putpixel((9, 5), colorsDict[primaryColor])
    imNew.putpixel((10, 5), colorsDict[primaryColor])
    imNew.putpixel((11, 5), colorsDict[primaryColor])
    imNew.putpixel((12, 5), colorsDict[primaryColor])
    imNew.putpixel((13, 5), colorsDict[primaryColor])
    imNew.putpixel((14, 5), colorsDict[primaryColor])
    imNew.putpixel((15, 5), colorsDict[primaryColor])
    imNew.putpixel((16, 5), colorsDict[primaryColor])
    imNew.putpixel((17, 5), colorsDict["black"])
    imNew.putpixel((18, 5), colorsDict["solanaBand"][5])
    imNew.putpixel((19, 5), colorsDict["solanaBand"][5])

    imNew.putpixel((4, 6), colorsDict["solanaBand"][6])
    imNew.putpixel((5, 6), colorsDict["solanaBand"][6])
    imNew.putpixel((6, 6), colorsDict["black"])
    imNew.putpixel((7, 6), colorsDict[strapColorShade])
    imNew.putpixel((8, 6), colorsDict[strapColor])
    imNew.putpixel((9, 6), colorsDict[strapColor])
    imNew.putpixel((10, 6), colorsDict[strapColor])
    imNew.putpixel((11, 6), colorsDict[buckleColor])
    imNew.putpixel((12, 6), colorsDict[strapColor])
    imNew.putpixel((13, 6), colorsDict[buckleColor])
    imNew.putpixel((14, 6), colorsDict[strapColor])
    imNew.putpixel((15, 6), colorsDict[strapColor])
    imNew.putpixel((16, 6), colorsDict[strapColor])
    imNew.putpixel((17, 6), colorsDict["black"])
    imNew.putpixel((18, 6), colorsDict["solanaBand"][6])
    imNew.putpixel((19, 6), colorsDict["solanaBand"][6])

    imNew.putpixel((4, 7), colorsDict["solanaBand"][7])
    imNew.putpixel((5, 7), colorsDict["solanaBand"][7])
    imNew.putpixel((6, 7), colorsDict["black"])
    imNew.putpixel((7, 7), colorsDict["black"])
    imNew.putpixel((8, 7), colorsDict["black"])
    imNew.putpixel((9, 7), colorsDict["black"])
    imNew.putpixel((10, 7), colorsDict["black"])
    imNew.putpixel((11, 7), colorsDict["black"])
    imNew.putpixel((12, 7), colorsDict["black"])
    imNew.putpixel((13, 7), colorsDict["black"])
    imNew.putpixel((14, 7), colorsDict["black"])
    imNew.putpixel((15, 7), colorsDict["black"])
    imNew.putpixel((16, 7), colorsDict["black"])
    imNew.putpixel((17, 7), colorsDict["black"])
    imNew.putpixel((18, 7), colorsDict["solanaBand"][7])
    imNew.putpixel((19, 7), colorsDict["solanaBand"][7])

    imNew.putpixel((4, 8), colorsDict["solanaBand"][8])
    imNew.putpixel((5, 8), colorsDict["black"])
    imNew.putpixel((6, 8), colorsDict[primaryColorShade])
    imNew.putpixel((7, 8), colorsDict[primaryColor])
    imNew.putpixel((8, 8), colorsDict[primaryColor])
    imNew.putpixel((9, 8), colorsDict[primaryColor])
    imNew.putpixel((10, 8), colorsDict[primaryColor])
    imNew.putpixel((11, 8), colorsDict[primaryColor])
    imNew.putpixel((12, 8), colorsDict[primaryColor])
    imNew.putpixel((13, 8), colorsDict[primaryColor])
    imNew.putpixel((14, 8), colorsDict[primaryColor])
    imNew.putpixel((15, 8), colorsDict[primaryColor])
    imNew.putpixel((16, 8), colorsDict[primaryColor])
    imNew.putpixel((17, 8), colorsDict[primaryColor])
    imNew.putpixel((18, 8), colorsDict["black"])
    imNew.putpixel((19, 8), colorsDict["solanaBand"][8])

    imNew.putpixel((5, 9), colorsDict["solanaBand"][9])
    imNew.putpixel((6, 9), colorsDict["black"])
    imNew.putpixel((7, 9), colorsDict["black"])
    imNew.putpixel((8, 9), colorsDict["black"])
    imNew.putpixel((9, 9), colorsDict["black"])
    imNew.putpixel((10, 9), colorsDict["black"])
    imNew.putpixel((11, 9), colorsDict["black"])
    imNew.putpixel((12, 9), colorsDict["black"])
    imNew.putpixel((13, 9), colorsDict["black"])
    imNew.putpixel((14, 9), colorsDict["black"])
    imNew.putpixel((15, 9), colorsDict["black"])
    imNew.putpixel((16, 9), colorsDict["black"])
    imNew.putpixel((17, 9), colorsDict["black"])
    imNew.putpixel((18, 9), colorsDict["solanaBand"][9])

    im.paste(imNew, (0,0), mask=imNew)
